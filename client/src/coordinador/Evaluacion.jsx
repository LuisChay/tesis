...
const EvaluacionContinua = () => {
  ...
  const evaluacionesPaginadas = evaluaciones.slice(startIndexEvaluaciones, endIndexEvaluaciones);
  const totalPagesEvaluaciones = Math.ceil(evaluaciones.length / ITEMS_PER_PAGE);
  ...
  const guardarEvaluacion = async () => {
    ...
    if (!usuario_id || !tarea_id || !nota) {
      return Swal.fire("Faltan campos", "Completa todos los campos obligatorios", "warning");
    }
    ...
    try {
      const res = await fetch("http://localhost:5100/coord/create-evaluacion", {
        ...
        body: JSON.stringify({ ...form, evaluado_por: evaluadorId, usuario_id }),
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error);

      const tareaNombre = tareas.find((t) => t.id === Number(form.tarea_id))?.titulo || "—";
  ...
  const eliminarEvaluacion = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5100/coord/delete-evaluacion/${id}`, {
        method: "DELETE",
      });
...
const startEdit = (evaluacion) => {
  ...
  setEditData({
    tarea_id: evaluacion.tarea_id,  // Aseguramos que tarea_id está siendo cargado correctamente
...
const handleEditChange = (e) => {
  ...
  setEditData((prev) => ({
    ...prev,
    [name]: value,  // Esto actualiza la tarea_id cuando cambia el select
  }));
};
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5100/coord/update-evaluacion/${editId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      setEditId(null);
      Swal.fire("Actualizada", "Evaluación actualizada correctamente", "success");
    } catch (err) {
      Swal.fire("Error", err.message, "error");
    }
  };
...
<td className="px-4 py-3 border-b">
  {editId === e.id ? (
    ...
    <div className="flex justify-between items-center mt-4">
      <button
        onClick={() => cambiarPaginaEvaluaciones(currentPageEvaluaciones - 1)}
